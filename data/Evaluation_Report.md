...
**Update (2026-08-13):** this Playwright/fake-camera test is a synthetic simulation; the actual open question -- whether a REAL human presenting a REAL replay attack to a REAL camera gets caught end to end -- was tested for real this pass, `DEBUG_CHALLENGE=1` live logging, not a synthetic driver. Two real attack presentations (a video played on a second screen, held up to the camera; a printed/on-screen photo held in hand and moved continuously) were both caught, every attempt (2/2 and 3/3 respectively), via `check_passive_liveness()`'s majority-vote sampling (fail ratios observed: 11/11, 5/5, 3/5, 3/3, 3/3). `check_frame_loop_signature()` recorded 0 matches across every single attempt -- real confirmation that a moved/varied real attack defeats the frame-repetition heuristic exactly as expected by design, while the trained anti-spoof model is the layer actually doing the work. A genuine live face passed cleanly in the same session immediately after (0/6 flagged samples) -- a flagged attack attempt does not lock out the real user. This closes the specific gap this section's synthetic test could never resolve.
...
3. **CONFIRMED, UNMITIGATED: Demographic Bias**: Bias testing (day36_38_bias_testing.py, 80 CFP images across skin tone/gender/age subgroups) found real, measured gaps — quality pass rate 20.0% (Female) vs. 42.5% (Male); passive liveness pass rate 25.0% (Senior) vs. 46.2% (Middle-aged); skin tone gap present but smaller (26.9% Dark vs. 35.7% Light). Sample sizes (24-40 per subgroup) are large enough to treat as a real signal, not large enough for regulatory-grade certainty. No mitigation has been attempted — this should be disclosed to any client or reviewer, not treated as resolved.
   **CORRECTION (2026-08-13):** the skin_tone/gender/age labels behind every number above were never real. `day36_38_bias_testing.py`'s `ensure_demographics_file()` auto-generates `data/cfp_demographics.csv` from array-index arithmetic (`idx % 3` for skin tone, `idx // 2 % 2` for gender, `idx // 3 % 3` for age) when the file doesn't exist, explicitly as a placeholder "so the script runs instantly," meant to be manually corrected by a human looking at the actual photos. Git history shows the file was written once, in the commit that built this feature, and never touched again -- confirmed by checking the actual CSV against that formula row by row, an exact match. Separately, the same script computed passive-liveness pass/fail via `antispoof_score >= 0.90`, the identical score-threshold-vs-real-decision bug Section 5.1.3 already found and corrected for the screen-replay evaluation, never carried over here.
   Both fixed: all 40 identities' first frontal photo were actually viewed and annotated for real (apparent skin tone/gender presentation/age bracket), and the liveness pass/fail now uses `check_passive_liveness()`'s real `status`/`is_real`, not the score threshold. Re-run on the same 80 images:

   | Subgroup | Quality pass (real labels) | Passive liveness pass (real labels + real decision) |
   |---|---|---|
   | Dark (n=12) | 75.0% (9/12) | 58.3% (7/12) |
   | Light (n=46) | 82.6% (38/46) | 60.9% (28/46) |
   | Medium (n=22) | 86.4% (19/22) | 31.8% (7/22) |
   | Female (n=20) | 95.0% (19/20) | 80.0% (16/20) |
   | Male (n=60) | 78.3% (47/60) | 43.3% (26/60) |
   | Young (n=32) | 87.5% (28/32) | 50.0% (16/32) |
   | Middle-aged (n=32) | 71.9% (23/32) | 62.5% (20/32) |
   | Senior (n=16) | 93.8% (15/16) | 37.5% (6/16) |

   This is a real, different picture, not a smaller version of the same one. The skin-tone finding **inverts**: Medium, not Dark, now has the lowest liveness pass rate (31.8%), and Dark/Light are close to each other (58.3%/60.9%). The gender finding **reverses direction**: the original (fake-labeled) numbers claimed Female scored lower on quality; the real labels show the opposite on both quality (95.0% vs 78.3%) and liveness (80.0% vs 43.3%) -- Male is the lower-scoring group, not Female. One piece of the original analysis does hold up under real labels: the brightness root-cause (Section 6 item 3's original hypothesis) is confirmed -- real Dark-skin mean grayscale intensity measures 91.0, genuinely below `BRIGHTNESS_MIN=100`, vs. 123.0/122.2 for Light/Medium.

   **Sample expanded 2.5x (2026-08-13):** the real-annotation work above covered 40 identities; the same real-photo annotation process (each identity's photo actually viewed, not inferred) was extended to 60 more from the same CFP pool, without disturbing the original 40's selection or labels (verified: the deterministic seed-7 shuffle over the same 100-identity pool puts the same first 40 identities in the same order regardless of sample size, confirmed directly before proceeding). 100 identities / 200 images total. The larger sample **confirms the two directional reversals above rather than overturning them** -- gender: Female 93.8% quality / 79.7% liveness vs. Male 85.3% / 55.9% (same direction, gap narrower than the n=40 read, consistent with the smaller sample having overstated the gap's size, not its direction). Skin tone: Medium remains the lowest-liveness group (46.2%, was 31.8%), but Dark is now the *highest* (72.7%), not tied with Light as the n=40 sample suggested -- both real, both now on firmer statistical ground (Dark liveness sample size: 12 images -> 22; Female quality sample size: 20 -> 64). The brightness fix (Section 6 item 3's mitigation, below) also holds up: the Light-Dark p90 brightness gap measured 13.5 points on the smaller sample, 10.4 points on this larger one -- shrinking, not growing, with more data, the expected direction if the fix genuinely generalizes rather than having been tuned to the original 40. A separate, true apples-to-apples check (old mean-based metric vs. new p90-based metric, both computed against the exact same 100-identity/200-photo sample, rather than comparing across two different sample sizes) confirms the same conclusion more directly: the old metric's gap on this larger sample is 18.7 points, the new metric's is 10.4 -- a 44% relative reduction from the fix itself, isolated from any sample-size effect (`scratch/verify_n100_before_after.py`).
   Real, disclosed caveats on this corrected re-run, same discipline as everywhere else in this report: this was the same 40-identity CFP sample as before (not new data at the time), and it was a genuinely uneven sample once grouped correctly (30 Male/10 Female, 23 Light/11 Medium/6 Dark identities) -- smaller subgroups (Dark n=12, Female n=20) carried real sampling noise. As of the sample expansion below, this is now 100 identities (68 Male/32 Female, 63 Light/26 Medium/11 Dark) -- still uneven (reflecting the underlying CFP dataset's own real composition, not a sampling choice), still not the deliberately-balanced sample a formal fairness audit would need, but meaningfully firmer ground than the original 40, and the direction of every finding above held up under the larger sample rather than reversing again. CFP is celebrity/red-carpet photography (varied professional lighting, expressions, camera angles), not live selfie captures resembling what the deployed app actually processes -- these numbers describe this specific offline dataset, not a measurement of the live app's fairness in the field. No mitigation has been attempted for the gender gap or the small residual contrast effect -- same as before, this is a disclosed finding, not treated as resolved.

   **Mitigation attempt (2026-08-13):** the skin-tone brightness root cause -- `check_brightness()` measuring whole-image MEAN pixel intensity, which base skin reflectance dominates as much as scene lighting does -- was investigated for a real fix, not just left disclosed. Sweeping the existing mean-based threshold directly against this data confirmed it can't be tuned away: raising or lowering it moves every group's score together, including a synthetic underexposed negative control, because the same raw signal can't distinguish "darker skin" from "insufficient light." Switched the "too dark" side of the measurement (not "too bright" -- see `src/quality_checks.py`'s `BRIGHTNESS_MIN_P90` comment for why that side doesn't transfer) to the 90th-percentile pixel intensity (the face's brightest highlights -- cheekbone, forehead, nose bridge -- which exist under adequate light regardless of base skin tone) instead of the mean. Re-ran the full real evaluation end to end, same 80 images:

   | Metric | Before (mean) | After (p90 for darkness) |
   |---|---|---|
   | Dark brightness sub-score (mean) | 60.0 | **82.9** |
   | Light brightness sub-score (mean) | 80.4 | 96.4 |
   | Medium brightness sub-score (mean) | 74.6 | 88.8 |
   | Skin-tone brightness gap (Light − Dark) | 20.4 | **13.5** (34% smaller) |
   | Overall composite quality pass rate (all 80) | 82.5% | 87.5% |
   | Any subgroup's pass rate decreased | -- | **No** -- every skin-tone/gender/age subgroup's quality pass rate stayed the same or improved |

   Honest limit on this result: Dark's own **overall composite** quality pass rate stayed exactly 75.0% (9/12), unchanged -- the brightness sub-score improvement was real and large, but the other sub-checks that were never touched by this fix (blur 85.0, contrast 71.5, resolution 69.6 for Dark) still held those same 3 samples below the Balanced 70% threshold. This closes the specific, root-caused brightness-measurement confound with real, verified, no-regression data; it does not close the full quality-pass-rate gap, which has other, separate contributing factors this pass didn't address. Passive-liveness pass rates are unaffected (expected -- this fix only touches the quality-scoring path, not the anti-spoofing model). Real regression tests added (`tests/test_quality_and_liveness.py`) confirm the new metric still catches synthetically darkened images, not just uniformly passes more.

   **Follow-up investigation (2026-08-13): contrast and resolution checked the same way, no fix found or applied.** Blur was already ruled out -- Dark's blur sub-score (85.0 at the time) is higher than Light's and Medium's, so it isn't holding Dark back at all. Contrast and resolution were investigated directly against the raw per-photo measurements behind their sub-scores, not just the aggregate: at the sample size then available (n=6 Dark identities), raw contrast (grayscale standard deviation) measured nearly identical means across skin tones (Dark 55.6, Light 57.6), and resolution showed large per-photo variance (94-520px within Dark alone) driven by framing/cropping, not skin tone. Concluded at the time that neither had a fixable, systematic cause -- see the correction directly below, which found this understated the contrast finding specifically.

   **Correction, same day, after the sample expansion below:** re-checked contrast's raw signal against the expanded n=100 sample (Dark n=22 images): mean 54.37 vs. Light's 57.58, a difference of 3.21 with an effect size (difference / pooled standard deviation) of 0.25 -- a real, small, measurable gap, not zero. The composite contrast sub-score's stability across the sample expansion (71.5 at n=12 Dark images, still 71.5 at n=22) was itself evidence against "probably just noise" -- pure sampling noise would be expected to shift with a near-doubled sample, not hold steady. This is a genuine correction to the sentence above, not a restatement of it: there IS a small, real, non-zero contrast gap by skin tone. It remains unfixed, deliberately, for a different reason than "it isn't real" -- unlike brightness, which has a well-established physical mechanism (camera sensors reflecting less light from darker skin) that a p90-based redesign could be built around with confidence, this small a contrast effect (0.25 is conventionally "small," well below brightness's much larger, dominant effect) has no comparably established physical explanation on hand. Designing a compensating metric without a real mechanism to reason from would be closer to curve-fitting the correction to this dataset than principled engineering -- disclosed as a real, open, small effect rather than fixed on a guess. Resolution's conclusion (framing/cropping variance, no plausible skin-tone mechanism) is unaffected by this correction and still stands.
