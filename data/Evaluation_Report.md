...
3. **CONFIRMED, UNMITIGATED: Demographic Bias**: Bias testing (day36_38_bias_testing.py, 80 CFP images across skin tone/gender/age subgroups) found real, measured gaps — quality pass rate 20.0% (Female) vs. 42.5% (Male); passive liveness pass rate 25.0% (Senior) vs. 46.2% (Middle-aged); skin tone gap present but smaller (26.9% Dark vs. 35.7% Light). Sample sizes (24-40 per subgroup) are large enough to treat as a real signal, not large enough for regulatory-grade certainty. No mitigation has been attempted — this should be disclosed to any client or reviewer, not treated as resolved.
   **CORRECTION (2026-08-13):** the skin_tone/gender/age labels behind every number above were never real. `day36_38_bias_testing.py`'s `ensure_demographics_file()` auto-generates `data/cfp_demographics.csv` from array-index arithmetic (`idx % 3` for skin tone, `idx // 2 % 2` for gender, `idx // 3 % 3` for age) when the file doesn't exist, explicitly as a placeholder "so the script runs instantly," meant to be manually corrected by a human looking at the actual photos. Git history shows the file was written once, in the commit that built this feature, and never touched again -- confirmed by checking the actual CSV against that formula row by row, an exact match. Separately, the same script computed passive-liveness pass/fail via `antispoof_score >= 0.90`, the identical score-threshold-vs-real-decision bug Section 5.1.3 already found and corrected for the screen-replay evaluation, never carried over here.
   ...
   Real, disclosed caveats on this corrected re-run, same discipline as everywhere else in this report: this is the same 40-identity CFP sample as before (not new data), and it is now a genuinely uneven sample once grouped correctly (30 Male/10 Female, 23 Light/11 Medium/6 Dark identities) -- smaller subgroups (Dark n=12, Female n=20) carry real sampling noise a formal fairness audit would need a larger, deliberately balanced sample to resolve. CFP is celebrity/red-carpet photography (varied professional lighting, expressions, camera angles), not live selfie captures resembling what the deployed app actually processes -- these numbers describe this specific offline dataset, not a measurement of the live app's fairness in the field. No mitigation has been attempted for any of the real gaps above -- same as before, this is a disclosed finding, not treated as resolved.
   ...
   **Follow-up investigation (2026-08-13): contrast and resolution checked the same way, no fix found or applied.** Blur was already ruled out -- Dark's blur sub-score (85.0) is higher than Light's (63.2) and Medium's (61.3), so it isn't holding Dark back at all. Contrast and resolution were investigated directly against the raw per-photo measurements behind their sub-scores, not just the aggregate: raw contrast (grayscale standard deviation) measured nearly identical means across skin tones -- Dark 55.6, Light 57.6, Medium 56.9, a ~2-point spread with each group's own within-group range spanning 30+ points (31.8-82.9 for Dark alone) -- no systematic gap exists in the underlying signal the way brightness had one. Resolution (detected face bounding-box width in pixels) showed large real variance per photo (94-520px within the 6 real Dark identities alone) driven by how each individual photo happened to be framed and cropped, with no plausible physical mechanism linking that to skin tone. Neither has brightness's kind of real, generalizable, physically-explained cause -- attempting to tune either against this specific 6-identity Dark sample would risk fitting the fix to this one small sample's particular photos, the same overfitting risk already declined for the gender/contrast gap. No change made; this closes the investigation into whether a Phase 14-style fix exists for the remaining gap, with a real, checked "no" rather than leaving it unexamined.
