...
> **SAMPLE EXPANDED 2.5x, SAME DAY:** the real-photo annotation work was extended from 40 to 100 identities (200 images) from the same CFP pool, same methodology (each photo actually viewed, not inferred), without disturbing the original 40's selection or labels. The larger sample confirms rather than overturns the corrections above: the gender reversal holds (Female still scores higher on both quality and liveness, gap narrower than the small sample suggested), and the brightness fix's gap shrinks further (13.5 -> 10.4 points), the expected direction if it genuinely generalizes. One earlier conclusion needed its own correction: contrast's raw signal, re-checked at n=22 Dark images (was n=12), showed a real but small effect (0.25 effect size) that held steady rather than shrinking toward zero -- not "just noise" as first concluded, but still not fixed, since unlike brightness it has no established physical mechanism to design a principled correction around. Full detail in `data/Evaluation_Report.md`, Section 6 item 3.
